...
public class CsCacheManager implements CacheManager {

	private final CsCachingProvider cachingProvider;
	...
	public CsCacheManager(CsCachingProvider cachingProvider, Properties props, ClassLoader classLoader, URI uri) {
	...
	synchronized public <K, V, C extends Configuration<K, V>> Cache<K, V> createCache(String cacheName, C configuration)
		...
		if (cache == null) {
			cache = new CsCache<K, V>(this, cacheName, configuration);
